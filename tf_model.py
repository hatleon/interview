...
class TransE:
    def __init__(self, kg: KnowledgeGraph,
        ...
        with tf.variable_scope('embedding'):
            ...
            self.weight_embedding = tf.get_variable(name='weight',
    ...
    def build_graph(self):
        with tf.name_scope('normalization'):
            ...
            self.weight_embedding = tf.nn.l2_normalize(self.relation_embedding, dim=0)  // **Huang
    ...
    def infer(self, triple_pos, triple_neg):
        with tf.name_scope('lookup'):
            ...
            weight = tf.expand_dims(self.weight_embedding, -1)  // **Huang
            weight_pos = tf.matmul(relation_pos, weight)  // **Huang
            weight_neg = tf.matmul(relation_neg, weight)  // **Huang

        with tf.name_scope('link'):
            distance_pos = relation_pos + tail_pos - tf.multiply(weight_pos, head_pos)  // **Huang
            distance_neg = relation_neg + tail_neg - tf.multiply(weight_neg, head_neg)  // **Huang
